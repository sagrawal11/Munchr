...
export async function fetchMachinesFromDb() {
  ...
  try {
    ...
    let query = supabase
      ...
      .order('id');
    // Scope to this deployment's org once multi-tenancy is live (no filter pre-cutover), so the
    // consumer map and the operator dashboard/report only see this org's machines.

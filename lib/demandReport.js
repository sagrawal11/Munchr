...
export function computeDemandReport(events, machines, opts = {}) {
  ...
  const requests = events.filter(e => e.event_type === 'product_requested');
  ...
  // --- Direct requests: explicit "I wish this had X" submissions (strongest signal) ---
  const requestCounts = tallyBy(requests, e => e.query);
  const topRequests = topN(requestCounts, 10, 'product').map(item => ({
    ...item,
    machinesStocking: stock.machineCountForQuery(item.product),
  }));
  ...
  for (const e of [...searches, ...noResults, ...requests]) {
  ...
  // --- Per-item drilldown: where & when each searched product shows demand ---
  // Building attribution comes from search_performed (each carries the zone of its top result);
  // unmet (no_results_returned) searches have no building to attribute, but still count by hour.
  const itemAgg = {};
  const ensureItem = (q) => {
    if (!itemAgg[q]) {
      itemAgg[q] = { product: q, total: 0, unmet: 0, requested: 0, byBuilding: {}, byHour: Array.from({ length: 24 }, () => 0) };
    }
    return itemAgg[q];
  };
  for (const e of searches) {
    if (!e.query) continue;
    const it = ensureItem(e.query);
    it.total += 1;
    if (e.building_id) it.byBuilding[e.building_id] = (it.byBuilding[e.building_id] || 0) + 1;
    if (e.timestamp) { const hh = new Date(e.timestamp).getHours(); if (hh >= 0 && hh < 24) it.byHour[hh] += 1; }
  }
  for (const e of noResults) {
    if (!e.query) continue;
    const it = ensureItem(e.query);
  ...
  const itemBreakdowns = Object.values(itemAgg)
    ...
    .map(it => ({
      ...
      machinesStocking: stock.machineCountForQuery(it.product),
      topBuildings: topN(it.byBuilding, 6, 'building'),
      byHour: it.byHour.map((count, hour) => ({ hour, count })),
    }));
  ...
  // Direct requests carry extra weight — someone took the time to ask. Prioritize those
  // not stocked nearby (a +1000 boost keeps explicit requests above inferred demand).
  for (const item of topRequests) {
    if (item.count < cfg.minSearchesForRec) continue;
    if (item.machinesStocking === 0) {
      recommendations.push({
        type: 'add_requested',
        priority: 1000 + item.count,
        text: `"${item.product}" was directly requested ${item.count} time${item.count !== 1 ? 's' : ''} and isn't stocked here — a clear add.`,
      });
    } else {
      recommendations.push({
        type: 'expand_requested',
        priority: 500 + item.count,
        text: `"${item.product}" was directly requested ${item.count} time${item.count !== 1 ? 's' : ''}; it's stocked in ${item.machinesStocking} machine${item.machinesStocking !== 1 ? 's' : ''} — consider adding it here.`,
      });
    }
  }
  ...
  return {
    ...
    headline: {
      ...
      directionsClicks: directionsClicks.length, // raw count only — not a conversion metric
      totalUnmetSearches,

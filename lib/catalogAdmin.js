...
export async function addProductToMachine(supabase, machineId, productName, orgId = null) {
  ...
  // Stamp the operator's org on the inventory link once multi-tenancy is live (null = unchanged).
  const invRow = { machine_id: machineId, product_id: prod.data.id, available: true, updated_at: new Date().toISOString() };
  if (orgId != null) invRow.org_id = orgId;

  const inv = await supabase
    .from('machine_inventory')
    .upsert(invRow, { onConflict: 'machine_id,product_id' })

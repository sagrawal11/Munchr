...
export async function commitCatalog(supabase, catalog) {
  ...
  const invRows = catalog.inventory
    .map(i => ({ machine_id: refToMachineId(i.machineRef), product_id: productIdByName.get(norm(i.productName)), available: i.available, price: i.price ?? null, updated_at: new Date().toISOString() }))
  ...
  const saleRows = catalog.sales
    .map(s => ({ machine_id: refToMachineId(s.machineRef), product_id: productIdByName.get(norm(s.productName)), quantity: s.quantity ?? null, price: s.price ?? null, sold_at: s.soldAt || null, source: 'csv' }))
